...
const postSavedImages = async (req, res = response) => {
    const { email, iterations } = req.body;
    const limit = (12 * iterations) + 12; // get the number of images to return
    try {
        connection.query('SELECT i_id, i_linkmedium FROM image WHERE profile_p_email = ? AND i_saved = 1 AND i_bought = 0 LIMIT ?', [email, limit], (error, results) => {
            if (error) {
                console.error(error);
